...
span.special5p a:link,span.special5 a:visited{
...
<%
	...
	if(action != null && action.equals("monthdown")){
		...
		if (submitType.equals("下一步>>")){
			if(dayarray == null || dayarray.length == 0){
				...
				for(int i=0; i<opList.length; i++){//opList的长度代表有几组以日为基准的时间
			      if(opList[i].size()>maxNum){//数组的每一个元素就是一个链表
				  	maxNum = opList[i].size();
			      }
				}
				maxNum = maxNum - 1;
	
				if(maxNum>0){
					//session.setAttribute("num",maxNum);
					poll.setOptionNum(maxNum);
	
					String[][] timeov = new String[maxNum][dayarray.length];
					...
					for(int j=0; j<dayarray.length; j++){
					 ...
					 for(int i=0;i<dayarray.length; i++){
							...
							for(int j=0; j<opList.length; j++){
								String adayStr=(String)opList[j].get(0);
								if(dayarray[i][0]==Integer.parseInt(adayStr.substring(0,4)) && dayarray[i][1]==Integer.parseInt(adayStr.substring(5,adayStr.lastIndexOf(".")))-1 && dayarray[i][2]==Integer.parseInt(adayStr.substring(adayStr.lastIndexOf(".")+1))){
									for(int k=1; k<opList[j].size(); k++){
										timeov[k-1][i]=(String)opList[j].get(k);//存每天具体的时间
									}
								}
							}
					}
				//session.setAttribute("timeov",timeov);
					
					  poll.setTimeov(timeov);
				}
				 
				 
				 response.sendRedirect("adminSetTimeDetail.jsp");
				 return;
			}
		}
 ...
 <form name="calendarForm" method="post" action="adminSelectDate.jsp">
  <input type="hidden" name="action" value="selected"/>
 <div id="biaodan1">
    <span class="warning1">注意：重设选项后可能会对已投票的参加者的结果造成影响！</span><p></p>
		...
		%>
<span class="special">修改时间（共三步）：</span>修改参加者将能选择的时间方案
<p style="color:#484848;">按选您选择的日期，再点击一次，就可将它取消</p><br />

<span class="special5"><a href="adminSelectDate.jsp?action=monthdown">&lt;&lt;</a> 
     
		<span class="special3"><%=calendar.get(Calendar.YEAR) %>年&nbsp;<%=calendar.get(Calendar.MONTH)+1 %>月</span>
		<a href="adminSelectDate.jsp?action=monthup">&gt;&gt;</a></span>
<p><span class="special4">星期一</span><span class="special4">星期二</span><span class="special4">星期三</span><span class="special4">星期四</span><span class="special4">星期五</span><span class="special6">星期六</span><span class="special4">星期日</span>  </p><table class="table1">
 ...
 <form name="controlform" method="post" action="adminSelectDate.jsp">
